...
(defvar term-ansi-256-color-state nil)
(defvar term-ansi-256-color-mode nil)
...
(defun term-ansi-color-for (ground value)
  (cond
   ((< value 16)
    (let ((face (elt ansi-term-color-vector value)))
      (cond
       ((eq ground :foreground)
        (face-foreground face nil 'default))
       ((eq ground :background)
        (face-background face nil 'default)))))

   ((< value 232)
    (let* ((value (- value 16))
           (red (/ value 36))
           (green (/ (- value (* red 36)) 6))
           (blue (- value (+ (* red 36) (* green 6)))))
      (format "#%02x%02x%02x"
              (if (eq red 0) 0 (+ (* red 40) 55))
              (if (eq green 0) 0 (+ (* green 40) 55))
              (if (eq blue 0) 0 (+ (* blue 40) 55)))))
...
Entry to this mode runs the hooks on `term-mode-hook'."
  ...
  (make-local-variable 'term-ansi-256-color-state)
  (make-local-variable 'term-ansi-256-color-mode)
            ...
            (cond ((< term-terminal-state 2)
                  ...
                  ((eq term-terminal-state 2)     ; Seen Esc
                   (cond ((eq char ?\133)     ;; ?\133 = ?[
                         ...
                         ;;    (setq term-terminal-state 0)))
                         ((setq term-terminal-state 0))))
                  ((or (eq term-terminal-state 3)
                       (eq term-terminal-state 4)) ; Seen Esc [ or (
                   (cond ((and (>= char ?0) (<= char ?9))
                         ...
                         (t
                          (cond
                           ((eq term-terminal-state 3)
                            (term-handle-ansi-escape proc char))
                           ((eq term-terminal-state 4)
                            ;; FIXME: Actually handle ESC ( codes
                            ))
                          (setq term-terminal-more-parameters 0)
                          (setq term-terminal-previous-parameter-4 -1)
                          (setq term-terminal-previous-parameter-3 -1)
...
(defun term-handle-colors-array (parameter)
  (cond
   ...
   ;; In 256 colour mode, read colour mode as second argument
   ((and (integerp term-ansi-256-color-state) (not term-ansi-256-color-mode))
    (setq term-ansi-256-color-mode parameter)
    (setq term-ansi-face-already-done t))

   ;; In 256 colour mode, read colour value as third argument if mode is 5
   ((and (integerp term-ansi-256-color-state) (eq term-ansi-256-color-mode 5))
    (cond
     ((eq term-ansi-256-color-state 3)
      (setq term-ansi-current-color parameter))
     ((eq term-ansi-256-color-state 4)
      (setq term-ansi-current-bg-color parameter)))
    (setq term-ansi-256-color-state nil))
   ...
   ((eq parameter 38)
    (setq term-ansi-256-color-state 3)
    (setq term-ansi-256-color-mode nil)
    (setq term-ansi-face-already-done t))
   ...
   ((eq parameter 48)
    (setq term-ansi-256-color-state 4)
    (setq term-ansi-256-color-mode nil)
    (setq term-ansi-face-already-done t))
  ...
  (unless term-ansi-face-already-done
    (if term-ansi-current-invisible
        (let ((color (term-ansi-color-for
                      :foreground term-ansi-current-color)))
      ...
      (setq term-current-face
            (list :foreground
                  (term-ansi-color-for :foreground term-ansi-current-color)
                  :background
                  (term-ansi-color-for :background term-ansi-current-bg-color)
